...
type partitionConsumer struct {
	...
	// For Tencent TDMQ , UnmodifiableMap in java sdk
	metadata map[string]string
...
func newPartitionConsumer(parent Consumer, client *client, options *partitionConsumerOpts,
	...
	if tags, ok := options.tagMapTopicNames[partitionedTopicName]; ok {
		pc.metadata["TAGS"] = tags
	}
	if tagPattern, ok := options.tagPatternMapTopicNames[partitionedTopicName]; ok {
		pc.metadata["TAGS_PATTERN"] = tagPattern
	}
...
func (pc *partitionConsumer) internalBeforeReconsume(msg Message, reconsumeOptions ReconsumeOptions) (Producer, *ProducerMessage, string, error) {
	...
	prod := pc.dlq.getRetryProducer()
	desType := "retry"
	if reconsumeTimes > pc.dlq.policy.MaxDeliveries {
		propertiesMap["REAL_TOPIC"] = pc.dlq.policy.Topic
...
func (pc *partitionConsumer) grabConn() error {
	...
	// For Tencent TDMQ authcloud
	if len(pc.metadata) > 0 && pc.client.options.AuthCloud == nil {
		cmdSubscribe.Metadata = toKeyValues(pc.metadata)
	} else if pc.client.options.AuthCloud != nil {
		...
		metaMap["topic"] = pc.topic[strings.Index(pc.topic, "//")+2:]
		metaMap["clientId"] = strconv.FormatUint(pc.consumerID, 10)
		metaMap["requestId"] = strconv.FormatUint(requestID, 10)
		pc.client.options.AuthCloud.CreateAuthMetadata("consumerMessage", metaMap)
		cmdSubscribe.Metadata = toKeyValues(metaMap)
	}
...
func (pc *partitionConsumer) initializeCompressionProvider(compressionType pb.CompressionType) (compression.Provider, error) {
