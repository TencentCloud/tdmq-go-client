...
func (r *dlqRouter) getRetryProducer() Producer {
	r.producerLock.Lock()
	defer r.producerLock.Unlock()

	if r.retryProducer != nil {
		// Producer was already initialized
		return r.retryProducer
	}

	// Retry to create producer indefinitely
	backoff := &internal.Backoff{}
	for {
		retryProducer, err := r.client.CreateProducer(ProducerOptions{
			Topic:           r.policy.RetryTopic,
			CompressionType: LZ4,
			DisableBatching: true,
		})

		if err != nil {
			r.log.WithError(err).Error("Failed to create Retry producer")
			time.Sleep(backoff.Next())
			continue
		} else {
